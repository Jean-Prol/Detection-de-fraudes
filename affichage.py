...
def affichage_indicateur1(cat):
    ...
    fig, ax = plt.subplots()
    ax1 = plt.subplot(121)
    ax1.scatter([moy_X for _ in range(int(min(Y)), int(max(Y)+1))], [i for i in range(int(min(Y)), int(max(Y))+1)], label="Revenu moyen")
    ...
    ax2 = plt.subplot(122)
    ax2.scatter(M,M, label="Revenu = Dépenses")
    ax2.scatter(X, Z)
...
def affichage_indicateur23(cat):
    ...
    ax1.set_ylabel("Dépenses - Revenu")
    ax1.legend()
    fig.canvas.mpl_connect('button_press_event', af1)
    ax2 = plt.subplot(121)
    ax2.scatter([moy_X for _ in range(int(min(Y)), int(max(Y)+1))], [i for i in range(int(min(Y)), int(max(Y))+1)], label="Revenu moyen")
    ax2.scatter(M, [moy_Y for _ in range(int(min(X)), int(max(X))+1)], label="Patrimoine moyen")
    ax2.scatter(X, Y)
    ax2.scatter(M, [moy_Y-0.35*(moy_Y-min(Y)) for _ in range(int(min(X)), int(max(X))+1)], label="Patrimoine min + 0.65*écart moyen")
    ...
    fig.canvas.mpl_connect('button_press_event', af2)
    ax3 = plt.subplot(224)
    ax3.scatter(M,M, label="Revenu = Dépenses")
    ax3.scatter(X, Z)
